...
$(function(){
	...
	*/
	$('#add').click(function(){
		...
		//		  on('click',function(){$(this).html("JQUERY")});
		//JSON데이타 형식 사용]
		//$('#div').attr({'title':"여기를 클릭하세요",'class':'bg','style':'width:100px;height:100px;text-align:center;line-height:100px'}).click(function(){$(this).html("JQUERY");});
		
		//prop()함수 사용]
		$('#div').prop({'title':"Click Here!!!",'class':'bg','style':'width:100px;height:100px;text-align:center;line-height:100px'}).click(function(){$(this).html("JQUERY");});
	});
 	...
 	$('#read').click(function(){
		...
		console.log('a의 href속성(prop):',$("#atag").prop("href"));//http://localhost:9090/JQueryProj/Core/Attr.jsp#
 	
 		console.log('div의 class속성(prop):',$('#div').prop('class'));
 		console.log('div의 class속성(attr):',$('#div').attr('class'));
 		console.log("div의 id속성:",$('#div').get(0).id);
 		//$("선택자").get(0).class사용 불가 
		//고로 attr()함수나 prop()함수로 속성값을 읽어 온다
 		console.log("div의 class속성:",$('#div').get(0).class);//undefined
 	});
 	...
 	$('a:eq(0)').click(function(){
 		...
 		console.log('표기하지 않은 속성 href(get()):',$(this).get(0).href);//빈문자열
