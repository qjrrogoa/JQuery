...
<script>
	$(function() {
		...
		$(":input") : input태그뿐만 아니라 폼의 모든 하위요서 태그 선택
		*/
		console.log("input 태그 선택자 : ",$('input').length) //13
		console.log("폼 태그 선택자 : ",$(':input').length) //17
		console.log(":button 선택자 : ",$(':button').length) //3 <button>도 포함
				...
				case "값 읽기":
	                ...
	                */
	                $(':button').each(function(i){
	                	console.log('%d번쨰 버튼의 타입 : %s',i,$(this).prop('type'));
	                	if($(this).is('[type=button]')){
	                		console.log("%d번째 버튼의 value 속성 : %s",i,$(this).val());
	                	}
	                	else{//<button>태그
	                		//<button>태그일때는 val()함수로 일ㄱ지 못한다. value속성이 없다다
	                		console.log("%d번째 버튼의 타입 : %s",i,$(this).html());
	                	}
	                });
	                ...
	                console.log('type=text의 값 : ',$(':password').val());
	              	...
	              	//4]input type="checkbox"
	                    ...
	                    */
                    	console.log('체크된 총 요소 수 : ',$(":checked").length);
	 	                ...
	 	                //4-2] 체크된 것만 가져오기
	 	                 console.log("=======체크 된 것만 읽어오기 =========");
	 	                 console.log("1.checkbox:checked와 each()함수")
	 	                 $(':checkbox:checked').each(function(){
	 	                	 console.log($(this).val())
	 	                 });
	 	                console.log("2.checkbox filter()함수와 each()함수");
	 	                 $(':checkbox').filter(":checked").each(function(){
	 	                	 console.log($(this).val());
	 	                 })
	 	                 console.log("3.checkbox와 if로 체크된 것만 판단 그리고 each()함수");
	 	                 ...
	 	                 */
	 	                 ...
	 	                 console.log("type=hidden의 value:",$(":hidden").val());
	 	                 console.log("$(':input:hidden').val()",$(":input:hidden").val());
	 	                 console.log("$('input[type=hidden]').val()",$('input[type=hidden]').val());
	 	                 
	 	                 //7] input type = "image"
	 	                 //value값 얻어 올 수 없다 - value 속성 미 존재]
	 	                 //val()함수로 읽어 오면 빈 문자열이다
	 	                 
	 	                 console.log("$(\":image\").val() : %s",$(":image").val());//빈문자열
	 	                 
	 	                 //8]input type = "radio"
	 	                 //선택에 상관없이 무조건 첫번째 요소값만 읽어옴]
	 	                 console.log("$(':radio:checked').val() : %s",$(':radio:checked').val())
	 	                 //체크된 것만 얻기 : $(":radio:checked").each걸 필요 없다(하나만 얻어오니깐)
	 	                 
	 	                 //9]reset 및 submit버튼
	 	                 console.log("$(':reset').val() : %s",$(":reset").val());
	 	                 console.log("$(':sumbmit').val() : %s",$(":submit").length);
	 	                 console.log("$(':sumbmit').val() : %s",$(":submit").val());
	 	                 
	 	                 //10]select태그
	 	                 //$("select").val()-선택된 값을 바로 가져옴.
	 	                 //:select폼 선택자 없다.
	 	                 //console.log($(":select"));// 에러
	 	                 console.log('$("select").val() : %s',$("select").val());
	 	                 console.log('$(":selected").val() : %s',$(":selected").val());
